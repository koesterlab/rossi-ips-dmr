...
def get_valid_germ_layers_for_fgsea(wildcards):
    """
    For a given base wildcard, return the valid germ layers for fgsea:
    the 3 non-base layers plus the synthetic "all" category.
    This is used as a wildcard constraint to avoid Snakemake trying to
    run fgsea for the base layer itself.
    """
    return get_non_base_layers(wildcards.base) + ["all"]


rule fgsea_dmr_vs_diffexp:
    input:
        diffexp_vs_dmrs_promoter="results/{platform}/{caller}/base_{base}/{rna_data}/diffexp_vs_dmrs_{annotation_type}.tsv",
