...
sample_rows = []
unit_rows = []
fastqs = snakemake.input.new_fastqs

# Define contrast columns and their mappings
contrasts = {
    "ectoderm_vs_psc_new": ("ectoderm", "psc"),
    "endoderm_vs_psc_new": ("endoderm", "psc"),
    "mesoderm_vs_psc_new": ("mesoderm", "psc"),
    "ectoderm_vs_endoderm_new": ("ectoderm", "endoderm"),
    "ectoderm_vs_mesoderm_new": ("ectoderm", "mesoderm"),
    "mesoderm_vs_endoderm_new": ("mesoderm", "endoderm"),
}

for fastq in fastqs:
    sample_name = fastq.split("/")[-1].split(".")[0]
    relative_path = fastq.split("resources/")[1]
    cell_type = snakemake.params.labels_new[sample_name]
    rna_data = "new"

    # Build contrast columns
    contrast_values = {}
    for col_name, (pos_type, neg_type) in contrasts.items():
    ...
    contrast_values["just_get_counts"] = "+" if cell_type == "psc" else "-"
    sample_rows.append(
        {
