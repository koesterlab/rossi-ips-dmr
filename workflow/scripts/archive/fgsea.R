...
if (germ_layer == "all") {
  message("Using all germ layers for fgsea")
} else {
  message("Using only germ layer ", germ_layer, " for fgsea")
}
...
diffexp <- read_tsv(snakemake@input[["diffexp_vs_dmrs_promoter"]]) %>%
                  ...
                  # resolve multiple occurrences of the same ext_gene, usually
                  # meaning that several Ensembl genes map to the same gene
                  # symbol -- so we may lose resolution here, as long as gene
                  # symbols are used
                  group_by(ext_gene) %>%
...
# Genes are ranked by the combined methylation/expression score computed in
# compare_dmr_to_diffexp_with_tfs.py
